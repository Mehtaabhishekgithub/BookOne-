...
export const Navbar: React.FC = () => {
  ...
  const handleSwitchUser = (clerkId: string) => {
    ...
    if (clerkId === "user_mock_provider_01") {
      navigate("/dashboard");
    } else {
      navigate("/");
    }
  ...
  const handleSignOut = () => {
    localStorage.setItem("bookone_clerk_id", "signed_out");
    dispatch(apiSlice.util.resetApiState());
    setIsUserMenuOpen(false);
    navigate("/");
  };
    ...
    <header className="sticky top-0 z-50 glass border-b border-slate-200/80 transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-2 sm:gap-3">
          ...
          {!isSignedOut && user?.role === "PROVIDER" && (
            <Link
              to="/dashboard"
          ...
          {/* User Account / Sign In & Sign Out Dropdown */}
          <div className="relative ml-2">
            ...
            {isUserMenuOpen && (
              <div className="absolute right-0 mt-2 w-64 rounded-2xl bg-white border border-slate-200 shadow-xl p-2 animate-scale-up text-xs z-50">
                {isSignedOut ? (
                  <div className="p-3">
                    <p className="font-bold text-slate-900 text-sm mb-1">Sign In to BookOne</p>
                    <p className="text-slate-400 text-[11px] mb-3">Choose a demo account role:</p>
                    <div className="space-y-1.5">
                      <button
                        onClick={() => handleSwitchUser("user_mock_provider_01")}
                        className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold transition-colors"
                      >
                        <span>Sign In as Provider (Alex)</span>
                      </button>
                      <button
                        onClick={() => handleSwitchUser("user_mock_client_01")}
                        className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold transition-colors"
                      >
                        <span>Sign In as Client (Sarah)</span>
                      </button>
                    </div>
                  </div>
                ) : (
                    ...
                    <div className="space-y-0.5">
                      ...
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-left hover:bg-slate-50 ${
                          activeClerkId === "user_mock_provider_01" || (!activeClerkId && user?.role === "PROVIDER")
